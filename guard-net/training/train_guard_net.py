"""Training pipeline for GUARD-Net.
...
Usage:
    python -m guard_net.training.train_guard_net \\
        --data guard/data/kim2018/nbt4061_source_data.xlsx \\
        --output guard-net/weights/guard_net_best.pt \\
...
def train_phase(
    ...
    phase: int = 1,
    save_path: str = "guard_net_best.pt",
    embedding_cache: EmbeddingCache | None = None,
    device: torch.device | None = None,
    seed: int = 42,
    **overrides,
) -> tuple[GUARDNet, dict[str, list[float]]]:
    """Full training for one phase.

    Args:
        model: GUARDNet instance.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train GUARD-Net")
    ...
    parser.add_argument("--output", type=str, default="guard-net/weights/guard_net_best.pt")
    ...
    logger.info(
        "GUARD-Net: %d trainable params | RNA-FM: %s | RLPA: %s | MT: %s",
    ...
    logger.info(
        ...
        "with your DataLoaders. See guard-net/training/train_guard_net.py."
    )
