"""Benchmark and ablation evaluation for GUARD-Net.

Produces the ablation table comparing model configurations:
    - Heuristic (baseline, 0 params)
    - SeqCNN v1 (CNN only, ~65K params)
    - GUARD-Net: CNN only (sanity check = v1)
    - GUARD-Net: CNN + RNA-FM
    - GUARD-Net: CNN + RNA-FM + RLPA
    - GUARD-Net: CNN + RNA-FM + RLPA + multi-task
...
Usage:
    python -m guard_net.evaluation.benchmark \\
        --data guard/data/kim2018/nbt4061_source_data.xlsx \\
        --output guard-net/results/ablation.json
"""
...
ABLATION_CONFIGS = [
    {
        "name": "GUARD-Net: CNN only",
        "use_rnafm": False,
        "use_rloop_attention": False,
        "multitask": False,
    },
    {
        "name": "GUARD-Net: CNN + RNA-FM",
        "use_rnafm": True,
        "use_rloop_attention": False,
        "multitask": False,
    },
    {
        "name": "GUARD-Net: CNN + RNA-FM + RLPA",
        "use_rnafm": True,
        "use_rloop_attention": True,
        "multitask": False,
    },
    {
        "name": "GUARD-Net: CNN + RNA-FM + RLPA + MT",
        "use_rnafm": True,
        "use_rloop_attention": True,
        "multitask": True,
    },
